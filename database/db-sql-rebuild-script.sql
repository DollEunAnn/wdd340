...
ALTER TYPE public.account_type OWNER TO wdd340pgsql;
...
VALUES ('Client'),
    ('Employee'),
    ('Admin');
-- 3 Add `role_id` column to `account` table with default value of 1 (Client)
ALTER TABLE public.account
ADD COLUMN role_id integer NOT NULL DEFAULT 1;
-- 4 Create relationship between `account` and `role` tables
ALTER TABLE IF EXISTS public.account
ADD CONSTRAINT fk_role FOREIGN KEY (role_id) REFERENCES public.role (role_id) MATCH simple ON UPDATE CASCADE ON DELETE NO ACTION;
-- 5 Update `account` table to set `role_id` based on previous `account_type` values
UPDATE public.account
SET role_id = CASE
        WHEN account_type = 'Client' THEN 1
        WHEN account_type = 'Employee' THEN 2
        WHEN account_type = 'Admin' THEN 3
        ELSE 1 -- Default to Client if account_type is NULL or unrecognized
    END;
-- Drop column `account_type` from `account` table and add `role_id` column as a foreign key to `role` table
ALTER TABLE public.account DROP COLUMN account_type;